...
describe('CSS Variables Validation', () => {
  ...
  describe('Typography Variables', () => {
    it('should define font size scale with correct values', () => {
      ...
      Object.entries(expectedFontSizes).forEach(([size, value]) => {
        expect(cssContent).toMatch(
          new RegExp(`--font-size-${size}:\\s*${value.replace('.', '\\.')};`)
        );
      });
    });
    ...
    it('should define line height scale', () => {
      ...
      Object.entries(expectedLineHeights).forEach(([name, value]) => {
        expect(cssContent).toMatch(
          new RegExp(`--line-height-${name}:\\s*${value.replace('.', '\\.')};`)
        );
      });
    });
  ...
  describe('Font Family Configuration', () => {
    it('should define font family variables in :root using Next.js font variables', () => {
      expect(cssContent).toMatch(
        /--font-sans:\s*var\(--font-inter\),\s*system-ui,\s*sans-serif;/
      );
      expect(cssContent).toMatch(
        /--font-serif:\s*var\(--font-spectral\),\s*Georgia,\s*serif;/
      );
      expect(cssContent).toMatch(
        /--font-mono:\s*var\(--font-jetbrains-mono\),\s*monospace;/
      );
    });
  });
  ...
  describe('Base Styles', () => {
    ...
    it('should style headings with Inter font', () => {
      const headingSection =
        cssContent.match(
          /h1,\s*h2,\s*h3,\s*h4,\s*h5,\s*h6\s*{[\s\S]*?}/
        )?.[0] || '';

      expect(headingSection).toContain('font-family: var(--font-sans);');
      expect(headingSection).toContain(
    ...
    it('should define correct heading sizes', () => {
      expect(cssContent).toMatch(
        /h1\s*{[\s\S]*?font-size:\s*var\(--font-size-4xl\);[\s\S]*?}/
      );
      expect(cssContent).toMatch(
        /h2\s*{[\s\S]*?font-size:\s*var\(--font-size-3xl\);[\s\S]*?}/
      );
      expect(cssContent).toMatch(
        /h3\s*{[\s\S]*?font-size:\s*var\(--font-size-2xl\);[\s\S]*?}/
      );
      expect(cssContent).toMatch(
        /h4\s*{[\s\S]*?font-size:\s*var\(--font-size-xl\);[\s\S]*?}/
      );
      expect(cssContent).toMatch(
        /h5\s*{[\s\S]*?font-size:\s*var\(--font-size-lg\);[\s\S]*?}/
      );
      expect(cssContent).toMatch(
        /h6\s*{[\s\S]*?font-size:\s*var\(--font-size-base\);[\s\S]*?}/
      );
    });
  });
  ...
  describe('Utility Classes', () => {
    it('should define container class with max-width constraint', () => {
      const containerSection =
        cssContent.match(/\.container\s*{[\s\S]*?}/)?.[0] || '';
    ...
    it('should define font family utility classes', () => {
      expect(cssContent).toMatch(
        /\.font-sans\s*{[\s\S]*?font-family:\s*var\(--font-sans\);[\s\S]*?}/
      );
      expect(cssContent).toMatch(
        /\.font-serif\s*{[\s\S]*?font-family:\s*var\(--font-serif\);[\s\S]*?}/
      );
      expect(cssContent).toMatch(
        /\.font-mono\s*{[\s\S]*?font-family:\s*var\(--font-mono\);[\s\S]*?}/
      );
    });
    ...
    it('should define text size utility classes', () => {
      ...
      textSizes.forEach((size) => {
        expect(cssContent).toMatch(
          new RegExp(
            `\\.text-${size}\\s*{[\\s\\S]*?font-size:\\s*var\\(--font-size-${size}\\);[\\s\\S]*?}`
          )
        );
      });
    });
    ...
    it('should define line height utility classes', () => {
      ...
      lineHeights.forEach((height) => {
        expect(cssContent).toMatch(
          new RegExp(
            `\\.leading-${height}\\s*{[\\s\\S]*?line-height:\\s*var\\(--line-height-${height}\\);[\\s\\S]*?}`
          )
        );
      });
    });
